...
#   ./verify_modelscope_lfs_hash.sh --write-manifest DeepSeek-V3.2-Exp-w8a8.sha256.manifest
#   ./verify_modelscope_lfs_hash.sh --manifest DeepSeek-V3.2-Exp-w8a8.sha256.manifest --model-dir /data/models/DeepSeek-V3.2-Exp-w8a8
...
Usage:
  ...
  verify_modelscope_lfs_hash.sh --write-manifest MANIFEST [options]
  verify_modelscope_lfs_hash.sh --manifest MANIFEST --model-dir MODEL_DIR
...
Options:
  ...
  --manifest FILE          Offline manifest generated by --write-manifest.
                           The server does not need ModelScope access in this mode.
  --write-manifest FILE    Generate an offline manifest from the ModelScope repo and exit.
                           Run this on a machine that can access ModelScope.
...
the LFS OID recorded in the ModelScope repository or in an offline manifest.
EOF
}
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --manifest)
        ...
        ;;
      --write-manifest)
        [[ $# -ge 2 ]] || die "--write-manifest requires a value"
        WRITE_MANIFEST="$2"
        shift 2
        ;;
      --ref-dir)
...
verify_from_manifest() {
  ...
  [[ -f "${manifest_file}" ]] || die "manifest does not exist: ${manifest_file}"
  [[ -d "${model_dir}" ]] || die "MODEL_DIR does not exist: ${model_dir}"

  log "verifying LFS file sha256 from manifest: ${manifest_file}"
...
if [[ -n "${MANIFEST}" && -n "${WRITE_MANIFEST}" ]]; then
  die "--manifest and --write-manifest cannot be used together"
fi

if [[ -n "${MANIFEST}" ]]; then
  [[ -n "${MODEL_DIR}" ]] || die "MODEL_DIR is required with --manifest"
  need_cmd sha256sum
  verify_from_manifest "${MANIFEST}" "${MODEL_DIR}"
  exit 0
fi

if [[ -z "${WRITE_MANIFEST}" ]]; then
...
if [[ -n "${WRITE_MANIFEST}" ]]; then
  log "writing offline manifest: ${WRITE_MANIFEST}"
  mkdir -p "$(dirname "${WRITE_MANIFEST}")"
  {
    echo "# repo: ${REPO_URL}"
    echo "# revision: $(git rev-parse HEAD)"
    echo "# generated_at: $(date -u '+%Y-%m-%dT%H:%M:%SZ')"
    awk '{print $1 "  " $3}' "${lfs_list_file}"
  } > "${WRITE_MANIFEST}"
  echo "Manifest written: ${WRITE_MANIFEST}"
  rm -f "${lfs_list_file}" "${lfs_exclude_file}"
  exit 0
fi
