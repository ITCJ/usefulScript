...
echo "pd_transfer_backend=${PD_TRANSFER_BACKEND}"

if [[ "${PD_TRANSFER_BACKEND}" == "ascend" ]]; then
  mf_endpoint=${ASCEND_MF_STORE_URL#tcp://}
  echo "memfabric_store=${mf_endpoint} protocol=${ASCEND_MF_TRANSFER_PROTOCOL}"
fi
...
if [[ "${PD_TRANSFER_BACKEND}" == "ascend" ]]; then
  mf_host=${mf_endpoint%:*}
  mf_port=${mf_endpoint##*:}
  if wait_tcp_endpoint "${mf_host}" "${mf_port}" 2; then
    echo "READY ${mf_endpoint} (MemFabric config store)"
  else
    echo "DOWN  ${mf_endpoint} (MemFabric config store)"
  fi
fi
